...
export class Simulation {
 ...
 prepareTestState({ fullWater = false } = {}) {
  // Re-entering the code is idempotent when the pipe is already packed.
  if (this.bud <= 0) {
  ...
  this.picked = ['pipe', 'lighter', 'bottle', 'bag'];
  this.prep = 2;
  this.heat = 0;
  this.progress = 0;
  this.cap = false;
  this.outlet = true;
  this.water = fullWater ? 1.0 : 0;
  this.bud = 1;
  this.embers = 0;
  this.smoke = 0;
  this.lastQuality = 0;
  ...
  this.flow = 0;
  this.flameQuality = 0;
  this.notice = fullWater ? 'Prepared with full water.' : 'Prepared.';
  this.noticeTimer = 3;
  this.assert();
  return true;
 }
 ...
 // `smoke` is the fraction of the charge converted into trapped smoke, not a
 // literal bottle-volume fraction. This lets a half-smoked bowl occupy a
 // quarter-bottle headspace as a denser mixture. The short reveal ramp keeps
 // the first few wisps from flashing to full optical density.
 get smokeDensity(){
  if(this.smoke<=0)return 0;
  const concentration=clamp(this.smoke/Math.max(.015,1-this.water));
  return clamp(concentration*smoothstep01(this.smoke/.12));
 }
 ...
 toggle(id){
  if(id==='stream'){
   if(this.mode==='fill'){this.mode='idle';return true;}
   return this.action('stream');
  }
  if(this.isHeld(id))return this.stow(id);
 ...
 step(dt,input={}){
  ...
  this.flameQuality=input.fire&&this.held==='lighter'&&(lighterAlone||['heat','hole','ignite'].includes(this.mode))?(lighterAlone?1:aim):0;
  if(this.phase==='heat'&&this.mode==='heat'&&this.supporting==='pipe'){
   ...
   if(this.mode==='unscrew'){
    ...
    if(this.progress<=0){
     this.setCap(false);
     this.supporting=null;
     if(this.held==='pipe')this.held='bottle';
     this.phase='hole';
     this.progress=0;
     this.say('Cap set down. Select the lighter to form the lower opening.');
    }
   }
   ...
   if(tilt<0){
    if(this.cap){
     ...
     if(this.mode!=='uncap'){this.mode='uncap';this.progress=1;}
    }
   }else if(tilt>0){
    if(!this.cap){
     if(this.held!=='bottle'){this.held='bottle';this.supporting=null;}
     if(this.mode!=='screw'){this.mode='screw';this.progress=0;if(this.prep>0){this.supporting='pipe';if(!this.picked.includes('pipe'))this.picked.push('pipe');}}
    }
   }
   ...
   if(this.outlet&&!sealed&&this.flow>0&&this.cap&&this.bud>0&&(lighting>0||this.embers>.08)){
    const embNorm=Math.max(0,(this.embers-.08)/.92);
    // At full flame contact, releasing 25 percentage points of water burns
    // half the bowl; releasing 50 points burns the full bowl. Partial contact
    // scales linearly. A small ember-only carry keeps an established cherry
    // alive without overpowering the player's current flame aim.
    const emberCarry=lighting>0?0:Math.pow(embNorm,1.35)*.22;
    const combustionContact=clamp(lighting+emberCarry);
    const burned=Math.min(this.bud,this.flow*2*combustionContact);
    this.bud-=burned;
    this.smoke=clamp(this.smoke+burned);
