...
await fs.rename(path.join(output,'electron.exe'),path.join(output,'Znicz.exe')).catch(async e=>{if(e.code!=='EEXIST'&&e.code!=='ENOENT')throw e;});
await fs.rename(path.join(output,'Stillwater.exe'),path.join(output,'Znicz.exe')).catch(async e=>{if(e.code!=='EEXIST'&&e.code!=='ENOENT')throw e;});
...
await fs.writeFile(path.join(appDir,'package.json'),JSON.stringify({name:'znicz',productName:'ZNICZ',version:'1.0.0',main:'desktop.cjs',description:'A quiet first-person forest ritual.'},null,2));
...
console.log('Portable Windows app:',path.join(output,'Znicz.exe'));
