...
const outDir = path.resolve('qa/gh35-baseline');
...
try {
  ...
  await page.evaluate(() => {
    ...
    window.__game.world.setQuality('medium');
  });
  ...
  // 2. Shallow-water bottom view (looking into shallow water)
  ...
  const manifest = await session.finalize({
    warning: 'GH-35 streambed baseline captures before 3D rebuild.',
    errors,
    extraMeta: {
      scope: 'GH-35 baseline: exposed edge, shallow water bottom, along stream, across stream, refill view'
    }
  });
