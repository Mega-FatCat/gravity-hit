...
- **`[AUTOMATED VERIFIED]` Measured Medium cost:** `qa/atm-01/benchmark.json` records two off and two on runs of 180 frames per view on RTX 3070 / WebGL2 at 1920x1080. Average GPU on/off times: canopy 25.380/25.324 ms; stream 55.710/55.771 ms; rocky bed 52.609/51.925 ms. The net difference is noise-level in canopy/stream and +0.684 ms for the rocky bed. Every paired view adds exactly one draw and two fullscreen triangles. Current total stream performance is ~18 FPS with either setting; this independent geometry-heavy baseline must not be described as ATM-01 cost. Details are in `performance-summary.json`.
...
- **`[CURRENT BUILD NEEDS MANUAL CHECK]` Concurrent work and limits:** The true pre-ATM `qa/atm-01/baseline/` records the older 6,613-stone scene. STREAM-BED-01 was independently changed during this session to the later configuration recorded above. Use the final same-build off/on pairs to assess atmospheric regression, not cross-version stone differences. Protected foliage rendering, edge-quality, props and smoke sources remain unmodified by ATM-01. Slow-camera stills support inspection but cannot establish full interactive freedom from shimmer; the human user's check remains required.
